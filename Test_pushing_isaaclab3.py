...
SETTLE_FRAMES      = 10
MAX_SETTLE_FRAMES  = 900   # cap on waiting for the pad to go quiet (~15 s @60Hz)
BASELINE_FRAMES    = 10
END_FRAMES         = 10
        ...
        if phase == "baseline":
            ...
            if deform_view is not None and rest_ref is not None:
                if not settle_ok:
                    ...
                    _w = settle_hist[-5:]
                    if len(settle_hist) >= 5 and (max(_w) - min(_w)) < 5e-5 and max(_w) < 1e-3:
                        settle_ok = True
